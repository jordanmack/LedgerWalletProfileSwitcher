<#
Switch the Ledger Live data folder (junction) between profiles.
Profiles are folders like:
  %APPDATA%\Ledger Live Profile 1
  %APPDATA%\Ledger Live Profile 2
The junction at %APPDATA%\Ledger Live points directly to the chosen profile.
#>

# ========= SETTINGS (edit if needed) =========
$BasePath     = Join-Path $env:APPDATA 'Ledger Live'                  # Junction path Ledger Live uses
$ProfilesBase = $env:APPDATA                                          # Where "Ledger Live Profile *" folders live
$LedgerExe    = 'C:\Program Files\Ledger Live\Ledger Live.exe'        # Ledger Live executable
$LaunchTimeoutSec = 30                                                # How long to wait for LL to appear
# ============================================

# Suppress confirmation prompts
$ConfirmPreference = 'None'

Write-Host '=== SwitchLedgerProfile.ps1 ===' -ForegroundColor Cyan
Write-Host ''

# Verify Ledger Live executable
if (-not (Test-Path -LiteralPath $LedgerExe)) {
    Write-Host "Error: Ledger Live not found at '$LedgerExe'." -ForegroundColor Red
    exit 1
}
...
$profiles = Get-ChildItem -Path $ProfilesBase -Directory |
    Where-Object { $_.Name -like 'Ledger Live Profile*' } |
    Sort-Object Name

if (-not $profiles -or $profiles.Count -eq 0) {
    Write-Host "No profile folders found under: $ProfilesBase" -ForegroundColor Yellow
    Write-Host "Create folders like:"
    Write-Host "  $ProfilesBase\Ledger Live Profile 1"
    Write-Host "  $ProfilesBase\Ledger Live Profile 2"
    exit 1
}
...
# If already pointing to this profile, skip switching
try {
    ...
    if ($curItem.Attributes -band [IO.FileAttributes]::ReparsePoint) {
        ...
        if ($curTarget) {
            ...
            if ($resolvedCur -eq $resolvedNew) {
                ...
                Write-Host ''
                Write-Host 'Launching Ledger Live...'
                Start-Process -FilePath $LedgerExe | Out-Null
                # Wait for Ledger Live to appear and then close when it exits
                $deadline = (Get-Date).AddSeconds($LaunchTimeoutSec)
                do {
                    $p = Get-Process -Name 'Ledger Live' -ErrorAction SilentlyContinue
                    if ($p) { break }
                    Start-Sleep -Milliseconds 200
                } while ((Get-Date) -lt $deadline)
                if ($p) { while (Get-Process -Name 'Ledger Live' -ErrorAction SilentlyContinue) { Start-Sleep 1 } }
                exit 0
            }
        }
    }
...
# Stop Ledger Live if running
Get-Process -Name 'Ledger Live' -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
...
# Launch Ledger Live and close when it exits
Write-Host ''
Write-Host 'Launching Ledger Live...'
Start-Process -FilePath $LedgerExe | Out-Null
...
$deadline = (Get-Date).AddSeconds($LaunchTimeoutSec)
do {
    $proc = Get-Process -Name 'Ledger Live' -ErrorAction SilentlyContinue
    if ($proc) { break }
    Start-Sleep -Milliseconds 200
} while ((Get-Date) -lt $deadline)

if ($proc) {
    while (Get-Process -Name 'Ledger Live' -ErrorAction SilentlyContinue) {
        Start-Sleep 1
    }
}
